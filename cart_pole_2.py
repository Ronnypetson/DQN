...
def get_argmaxes(a):
	return [np.argmax([a[i]]) for i in range(batch_size)]
...
with tf.Session() as sess:
	...
	for t in range(500000):
		...
		i = 0
		while not d:
			actQ = sess.run(fc3,feed_dict={X:[obs]})
			actQ = np.transpose(actQ)
			#print(actQ)
			a = env.action_space.sample() if np.random.rand(1) < e else np.argmax(actQ)
			new_obs,r,d = step(env,a) # render=(t/500 % 8 == 7)
			new_mem = {'q_sa': actQ[a],'obs':obs,'act':a,'r':r,'new_obs':new_obs,'d':d}
			replace_mem(new_mem)
			obs = new_obs
			i += 1
		scores.append(i)
		...
		#print(actQ)
		#
		argmaxQ = get_argmaxes(actQ) #
		#print(argmaxQ)
		#b_d = [b_d[j] for j in argmaxQ]
		#b_r = [b_r[j] for j in argmaxQ]
		#q_sa = [q_sa[j] for j in argmaxQ]
		y = sess.run(fc3,feed_dict={X:b_ob}) # np.zeros((batch_size,num_keys))
		for j in range(batch_size):
			a = b_act[j]
			if b_d[j]:
				y[j][a] = b_r[j]
			else:
				y[j][a] = b_r[j] + gamma*actQ[j][argmaxQ[j]]
